...
static inline uint64 address_space_vsid(addr_t ea, addr_t addressSpace)
{
	uint64 esid = segment_id(ea);
	if (addressSpace == 0 || IS_KERNEL_ADDRESS(ea)) return esid;
...
static inline uint64 vpn(addr_t ea, addr_t addressSpace)
	{ return (address_space_vsid(ea, addressSpace) << 16) | (((uint64)ea >> PPC64_PAGE_SHIFT) & 0xffff); }
...
static ppc64_pte* find(uint64 v, bool secondary)
{
	...
	for (uint32 i = 0; i < PPC64_HPT_PTES_PER_GROUP; i++) {
		...
		if ((value & PPC64_HPTE_V_VALID) && (value & PPC64_HPTE_V_AVPN_MASK)
			== (wanted & PPC64_HPTE_V_AVPN_MASK)) return &p[i];
	}
	return NULL;
}

static ppc64_pte* find_free_or_victim(ppc64_pte* p)
{
	for (uint32 i = 0; i < PPC64_HPT_PTES_PER_GROUP; i++)
		if (!(p[i].word0 & PPC64_HPTE_V_VALID)) return &p[i];
...
static status_t insert(addr_t ea, phys_addr_t pa, uint32 protection,
	uint32 memoryType, addr_t addressSpace)
{
	uint64 v = vpn(ea, addressSpace);
	uint64 h = hash(v);
	uint64 r = ((uint64)pa & PPC64_HPTE_R_RPN) | PPC64_HPTE_R_R;
	bool kernel = IS_KERNEL_ADDRESS(ea);
	if (kernel) r |= (protection & B_KERNEL_WRITE_AREA) ? PPC64_HPTE_PP_RWXX : PPC64_HPTE_PP_RXRX;
	else if (protection & B_WRITE_AREA) r |= PPC64_HPTE_PP_RWRW;
	else r |= PPC64_HPTE_PP_RWRX;
	if (memoryType != 0) r |= PPC64_HPTE_R_I | PPC64_HPTE_R_G;

	ppc64_pte* p = find_free_or_victim(group(h));
	if (p != NULL) {
		p->word1 = r; sync(); p->word0 = avpn(v) | PPC64_HPTE_V_VALID; eieio(); return B_OK;
	}
	p = find_free_or_victim(group(~h));
	if (p != NULL) {
		p->word1 = r; sync(); p->word0 = avpn(v) | PPC64_HPTE_V_H
			| PPC64_HPTE_V_SECONDARY | PPC64_HPTE_V_VALID; eieio(); return B_OK;
	}
	return B_NO_MEMORY;
}
...
{
	if (args == NULL || args->arch_args.page_table.start == 0) return B_ERROR;
	sHPT = (ppc64_pteg*)args->arch_args.page_table.start;
	sHPTSize = args->arch_args.page_table.size;
	if (sHPTSize < 256 * 1024 || (sHPTSize & (sHPTSize - 1)) != 0) return B_BAD_VALUE;
...
{
	if (sHPT == NULL) return B_NOT_INITIALIZED;
	...
	ppc64_slb_insert(slot, esid, vsid | (kernel ? PPC64_SLB_VSID_KP : 0));
	return B_OK;
}
	...
	uint32 protection, uint32 memoryType, addr_t addressSpace)
{
	...
	ppc64_pte* p = find(v, false); if (p == NULL) p = find(v, true);
	if (p != NULL) { p->word0 &= ~PPC64_HPTE_V_VALID; sync(); arch_cpu_invalidate_tlb_range(0, virtualAddress, virtualAddress + PPC64_PAGE_SIZE); }
	return insert(virtualAddress, physicalAddress, protection, memoryType, addressSpace);
}
status_t ppc64_map_page(addr_t virtualAddress, phys_addr_t physicalAddress,
	uint32 protection, uint32 memoryType)
	{ return ppc64_map_page_asid(virtualAddress, physicalAddress, protection, memoryType, sCurrentAddressSpace); }
...
status_t ppc64_unmap_page_asid(addr_t virtualAddress, addr_t addressSpace)
{
	...
	if (p == NULL) p = find(vpn(virtualAddress, addressSpace), true);
	if (p == NULL) return B_ENTRY_NOT_FOUND;
	p->word0 &= ~PPC64_HPTE_V_VALID; sync();
	arch_cpu_invalidate_tlb_range(0, virtualAddress, virtualAddress + PPC64_PAGE_SIZE);
	return B_OK;
}
...
status_t
ppc64_query_page_asid(addr_t virtualAddress, phys_addr_t* physicalAddress,
	uint32* protection, addr_t addressSpace)
{
	...
	if (p == NULL) p = find(vpn(virtualAddress, addressSpace), true);
	if (p == NULL) return B_ENTRY_NOT_FOUND;
	if (physicalAddress != NULL) *physicalAddress = p->word1 & PPC64_HPTE_R_RPN;
	if (protection != NULL) {
		uint32 pp = p->word1 & PPC64_HPTE_R_PP;
		*protection = pp == PPC64_HPTE_PP_RWXX ? B_KERNEL_READ_AREA | B_KERNEL_WRITE_AREA
			: pp == PPC64_HPTE_PP_RWRW ? B_READ_AREA | B_WRITE_AREA : B_READ_AREA;
	}
	return B_OK;
}

status_t
ppc64_clear_page_flags_asid(addr_t virtualAddress, uint32 flags,
	bool* modified, addr_t addressSpace)
{
	...
	if (p == NULL) return B_ENTRY_NOT_FOUND;
	uint64 old = p->word1;
	if (modified != NULL) *modified = (old & PPC64_HPTE_R_C) != 0;
	if (flags != 0) p->word1 &= ~(uint64)(PPC64_HPTE_R_R | PPC64_HPTE_R_C);
	eieio();
	return B_OK;
}
