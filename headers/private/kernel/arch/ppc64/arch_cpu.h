...
enum machine_state {
	MSR_EXCEPTIONS_ENABLED=1ULL<<15, MSR_PRIVILEGE_LEVEL=1ULL<<14,
	MSR_FP_AVAILABLE=1ULL<<13, MSR_MACHINE_CHECK_ENABLED=1ULL<<12,
	MSR_EXCEPTION_PREFIX=1ULL<<6, MSR_INST_ADDRESS_TRANSLATION=1ULL<<5,
	MSR_DATA_ADDRESS_TRANSLATION=1ULL<<4, MSR_RECOVERABLE_EXCEPTION=1ULL<<1,
	MSR_64BIT=1ULL<<63
};
...
extern "C" {
#endif
extern uint64 get_sdr1(void); extern void set_sdr1(uint64 value);
extern uint64 get_msr(void); extern void set_msr(uint64 value);
extern uint32 get_pvr(void); extern uint64 get_time_base(void);
