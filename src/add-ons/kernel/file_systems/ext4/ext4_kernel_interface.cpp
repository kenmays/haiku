...
static status_t
ext4_scan_partition(int fd, partition_data *partition, void *_cookie)
{
...
static void
ext4_free_identify_partition_cookie(partition_data* partition, void* _cookie)
{
...
static status_t
ext4_read_fs_info(fs_volume* _volume, struct fs_info* info)
{
	Volume* volume = (Volume*)_volume->private_volume;
...
static status_t
ext4_write_fs_info(fs_volume* _volume, const struct fs_info* info, uint32 mask)
{
	Volume* volume = (Volume*)_volume->private_volume;
...
static status_t
ext4_get_vnode(fs_volume* _volume, ino_t id, fs_vnode* _node, int* _type,
...
static status_t
ext4_put_vnode(fs_volume* _volume, fs_vnode* _node, bool reenter)
{
...
static status_t
ext4_remove_vnode(fs_volume* _volume, fs_vnode* _node, bool reenter)
{
	TRACE("ext4_remove_vnode()\n");
	Volume* volume = (Volume*)_volume->private_volume;
	...
	TRACE("ext4_remove_vnode(): Starting transaction\n");
	Transaction transaction(volume->GetJournal());

	if (!inode->IsSymLink() || inode->Size() >= EXT2_SHORT_SYMLINK_LENGTH) {
		TRACE("ext4_remove_vnode(): Truncating\n");
		status_t status = inode->Resize(transaction, 0);
		if (status != B_OK)
			return status;
	}

	TRACE("ext4_remove_vnode(): Setting deletion time\n");
	...
	TRACE("ext4_remove_vnode(): Freeing inode\n");
...
static bool
ext4_can_page(fs_volume* _volume, fs_vnode* _node, void* _cookie)
{
	return true;
}
...
static status_t
ext4_read_pages(fs_volume* _volume, fs_vnode* _node, void* _cookie,
...
static status_t
ext4_write_pages(fs_volume* _volume, fs_vnode* _node, void* _cookie,
	off_t pos, const iovec* vecs, size_t count, size_t* _numBytes)
{
	Volume* volume = (Volume*)_volume->private_volume;
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_get_file_map(fs_volume* _volume, fs_vnode* _node, off_t offset,
	size_t size, struct file_io_vec* vecs, size_t* _count)
{
	TRACE("ext4_get_file_map()\n");
	Volume* volume = (Volume*)_volume->private_volume;
	Inode* inode = (Inode*)_node->private_node;
	...
	while (true) {
		...
		if (block > volume->NumBlocks()) {
			panic("ext4_get_file_map() found block %" B_PRIu64 " for offset %"
				B_PRIdOFF "\n", block, offset);
		}
		...
		if (offset >= inode->Size() || size <= blockLength) {
			// We're done!
			*_count = index;
			TRACE("ext4_get_file_map for inode %" B_PRIdINO "\n", inode->ID());
			return B_OK;
		}
...
static status_t
ext4_lookup(fs_volume* _volume, fs_vnode* _directory, const char* name,
	ino_t* _vnodeID)
{
	TRACE("ext4_lookup: name address: %p\n", name);
	TRACE("ext4_lookup: name: %s\n", name);
	Volume* volume = (Volume*)_volume->private_volume;
...
static status_t
ext4_ioctl(fs_volume* _volume, fs_vnode* _node, void* _cookie, uint32 cmd,
...
*/
static status_t
ext4_set_flags(fs_volume* _volume, fs_vnode* _node, void* _cookie, int flags)
{
...
static status_t
ext4_fsync(fs_volume* _volume, fs_vnode* _node, bool dataOnly)
{
	Inode* inode = (Inode*)_node->private_node;
	return inode->Sync();
}
...
status_t
ext4_write_stat(fs_volume* _volume, fs_vnode* _node, const struct stat* stat,
	uint32 mask)
{
	TRACE("ext4_write_stat\n");
	Volume* volume = (Volume*)_volume->private_volume;
	...
	if ((mask & B_STAT_SIZE) != 0 && inode->Size() != stat->st_size) {
		...
		TRACE("ext4_write_stat: Old size: %ld, new size: %ld\n",
...
static status_t
ext4_create(fs_volume* _volume, fs_vnode* _directory, const char* name,
	int openMode, int mode, void** _cookie, ino_t* _vnodeID)
{
	...
	TRACE("ext4_create(): Creating cookie\n");
	...
	TRACE("ext4_create(): Starting transaction\n");

	Transaction transaction(volume->GetJournal());

	TRACE("ext4_create(): Creating inode\n");
	...
	TRACE("ext4_create(): Created inode\n");
...
static status_t
ext4_create_symlink(fs_volume* _volume, fs_vnode* _directory, const char* name,
	const char* path, int mode)
{
	TRACE("ext4_create_symlink()\n");
	...
	TRACE("ext4_create_symlink(): Starting transaction\n");
	Transaction transaction(volume->GetJournal());
	...
	TRACE("ext4_create_symlink(): Path (%s) length: %d\n", path, (int)length);
	...
	TRACE("ext4_create_symlink(): Done\n");
...
static status_t
ext4_link(fs_volume* volume, fs_vnode* dir, const char* name, fs_vnode* node)
{
	// TODO
...
static status_t
ext4_unlink(fs_volume* _volume, fs_vnode* _directory, const char* name)
{
	TRACE("ext4_unlink()\n");
	...
	TRACE("ext4_unlink(): Starting transaction\n");
	Transaction transaction(volume->GetJournal());

	directory->WriteLockInTransaction(transaction);

	TRACE("ext4_unlink(): Looking up for directory entry\n");
	HTree htree(volume, directory);
...
static status_t
ext4_rename(fs_volume* _volume, fs_vnode* _oldDir, const char* oldName,
	fs_vnode* _newDir, const char* newName)
{
	...
	TRACE("ext4_rename(): found entry to rename\n");

	if (oldDirectory != newDirectory) {
		TRACE("ext4_rename(): Different parent directories\n");
		CachedBlock cached(volume);
	...
	TRACE("ext4_rename(): found new directory\n");
	...
	if (status == B_OK) {
		TRACE("ext4_rename(): found existing new entry\n");
		if (existentID == oldID) {
...
static status_t
ext4_open(fs_volume* _volume, fs_vnode* _node, int openMode, void** _cookie)
{
	Volume* volume = (Volume*)_volume->private_volume;
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_read(fs_volume* _volume, fs_vnode* _node, void* _cookie, off_t pos,
...
static status_t
ext4_write(fs_volume* _volume, fs_vnode* _node, void* _cookie, off_t pos,
	const void* buffer, size_t* _length)
{
	...
	TRACE("ext4_write(): Preparing cookie\n");
	...
	TRACE("ext4_write(): Creating transaction\n");
...
static status_t
ext4_close(fs_volume *_volume, fs_vnode *_node, void *_cookie)
{
	return B_OK;
}
...
static status_t
ext4_free_cookie(fs_volume* _volume, fs_vnode* _node, void* _cookie)
{
	file_cookie* cookie = (file_cookie*)_cookie;
	Volume* volume = (Volume*)_volume->private_volume;
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_access(fs_volume* _volume, fs_vnode* _node, int accessMode)
{
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_read_link(fs_volume *_volume, fs_vnode *_node, char *buffer,
	size_t *_bufferSize)
{
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_create_dir(fs_volume* _volume, fs_vnode* _directory, const char* name,
	int mode)
{
	TRACE("ext4_create_dir()\n");
	Volume* volume = (Volume*)_volume->private_volume;
	Inode* directory = (Inode*)_directory->private_node;
	...
	TRACE("ext4_create_dir(): Starting transaction\n");
	Transaction transaction(volume->GetJournal());
...
static status_t
ext4_remove_dir(fs_volume* _volume, fs_vnode* _directory, const char* name)
{
	TRACE("ext4_remove_dir()\n");
	...
	TRACE("ext4_remove_dir(): Starting transaction\n");
	Transaction transaction(volume->GetJournal());

	directory->WriteLockInTransaction(transaction);

	TRACE("ext4_remove_dir(): Looking up for directory entry\n");
	HTree htree(volume, directory);
	DirectoryIterator* directoryIterator;
...
static status_t
ext4_open_dir(fs_volume* _volume, fs_vnode* _node, void** _cookie)
{
	Inode* inode = (Inode*)_node->private_node;
...
static status_t
ext4_read_dir(fs_volume *_volume, fs_vnode *_node, void *_cookie,
...
static status_t
ext4_rewind_dir(fs_volume * /*_volume*/, fs_vnode * /*node*/, void *_cookie)
{
	DirectoryIterator *iterator = (DirectoryIterator *)_cookie;
...
static status_t
ext4_close_dir(fs_volume * /*_volume*/, fs_vnode * /*node*/, void * /*_cookie*/)
{
	return B_OK;
}
...
static status_t
ext4_free_dir_cookie(fs_volume *_volume, fs_vnode *_node, void *_cookie)
{
...
static status_t
ext4_open_attr_dir(fs_volume *_volume, fs_vnode *_node, void **_cookie)
{
	Inode* inode = (Inode*)_node->private_node;
	Volume* volume = (Volume*)_volume->private_volume;
	TRACE("%s()\n", __FUNCTION__);
...
static status_t
ext4_close_attr_dir(fs_volume* _volume, fs_vnode* _node, void* cookie)
{
	TRACE("%s()\n", __FUNCTION__);
	return B_OK;
}
...
static status_t
ext4_free_attr_dir_cookie(fs_volume* _volume, fs_vnode* _node, void* _cookie)
{
	TRACE("%s()\n", __FUNCTION__);
...
static status_t
ext4_read_attr_dir(fs_volume* _volume, fs_vnode* _node,
...
static status_t
ext4_rewind_attr_dir(fs_volume* _volume, fs_vnode* _node, void* _cookie)
{
	*(int32*)_cookie = 0;
	TRACE("%s()\n", __FUNCTION__);
	return B_OK;
...
static status_t
ext4_open_attr(fs_volume* _volume, fs_vnode* _node, const char* name,
...
static status_t
ext4_close_attr(fs_volume* _volume, fs_vnode* _node,
	void* cookie)
{
	return B_OK;
}
...
static status_t
ext4_free_attr_cookie(fs_volume* _volume, fs_vnode* _node,
	void* cookie)
{
...
static status_t
ext4_read_attr(fs_volume* _volume, fs_vnode* _node, void* _cookie,
...
static status_t
ext4_read_attr_stat(fs_volume* _volume, fs_vnode* _node,
...
fs_vnode_ops gExt2VnodeOps = {
	...
	&ext4_open_attr_dir,
	&ext4_close_attr_dir,
	&ext4_free_attr_dir_cookie,
	&ext4_read_attr_dir,
	&ext4_rewind_attr_dir,

	/* attribute operations */
	NULL,
	&ext4_open_attr,
	&ext4_close_attr,
	&ext4_free_attr_cookie,
	&ext4_read_attr,
	NULL,
	&ext4_read_attr_stat,
	NULL,
...
static file_system_module_info sExt4FileSystem = {
	...
	// scanning
	ext4_identify_partition,
	ext4_scan_partition,
	ext4_free_identify_partition_cookie,
