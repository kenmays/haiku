...
void ppc_set_current_cpu_exception_context(ppc_cpu_exception_context* c)
	{ asm volatile("mtsprg0 %0" :: "r"(c)); }
status_t arch_smp_init(kernel_args*) { return AppleG5::mpic_init(); }
status_t arch_smp_per_cpu_init(kernel_args*, int32 cpu)
{
	ppc_cpu_exception_context* c = ppc_get_cpu_exception_context(cpu);
	if (!c) return B_BAD_VALUE;
	c->exception_context = c;
	ppc_set_current_cpu_exception_context(c);
...
void arch_smp_send_multicast_ici(CPUSet&)
{
#if KDEBUG
...
#endif
	int32 current = smp_get_current_cpu();
	for (int32 cpu = 0; cpu < smp_get_num_cpus(); cpu++)
		if (cpu != current) AppleG5::mpic_send_ipi(cpu, kIpiVector);
}
