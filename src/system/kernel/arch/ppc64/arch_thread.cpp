...
extern "C" void ppc64_enter_userspace(addr_t entry, addr_t stack,
	addr_t arg1, addr_t arg2, addr_t tls, addr_t returnAddress)
	__attribute__((noreturn));

void ppc_push_iframe(iframe_stack* s, iframe* f)
{
	ASSERT(s->index < IFRAME_TRACE_DEPTH);
	s->frames[s->index++] = f;
}

void ppc_pop_iframe(iframe_stack* s)
{
	ASSERT(s->index > 0);
	--s->index;
}
...
status_t arch_thread_init(kernel_args*)
{
	memset(&sInitialState, 0, sizeof(sInitialState));
	return B_OK;
}
...
{
	memcpy(&t->arch_info, &sInitialState, sizeof(arch_thread));
	return B_OK;
}
...
status_t arch_thread_enter_userspace(Thread* t, addr_t entry, void* a1, void* a2)
{
	...
	addr_t cp = (addr_t)t->team->commpage_address;
	addr_t exitAddr;
	status_t e = user_memcpy(&exitAddr,
		&((addr_t*)cp)[COMMPAGE_ENTRY_PPC64_THREAD_EXIT], sizeof(exitAddr));
	if (e != B_OK) return e;
	exitAddr += cp;
	...
	ppc64_enter_userspace(entry, sp, (addr_t)a1, (addr_t)a2,
		t->user_local_storage, exitAddr);
	return B_ERROR;
}

bool arch_on_signal_stack(Thread*) { return false; }
status_t arch_setup_signal_frame(Thread*, struct sigaction*, struct signal_frame_data*)
...
void arch_store_fork_frame(struct arch_fork_arg* arg)
{
	...
	if (frame == NULL) {
		memset(&arg->frame, 0, sizeof(arg->frame));
		return;
	}
...
{
	ppc64_restore_iframe(&arg->frame);
}
