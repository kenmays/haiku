...
void arch_smp_send_multicast_ici(CPUSet&set){
#if KDEBUG
if(are_interrupts_enabled())panic("arch_smp_send_multicast_ici: interrupts enabled");
#endif
int32 current=smp_get_current_cpu();for(int32 cpu=0;cpu<smp_get_num_cpus();cpu++)if(cpu!=current&&set.GetBit(cpu))AppleG5::mpic_send_ipi(cpu,kIpiVector);}
