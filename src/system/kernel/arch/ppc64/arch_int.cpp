...
static void handle_syscall(iframe* f)
{
	uint8 buffer[MAX_SYSCALL_PARAMETERS * 8]; memset(buffer, 0, sizeof(buffer));
	uint64 regs[6] = {f->r3, f->r4, f->r5, f->r6, f->r7, f->r8};
	uint32 number = (uint32)f->r0;
	if (number >= (uint32)kSyscallCount) { f->r3 = B_BAD_VALUE; return; }
	const extended_syscall_info& info = kExtendedSyscallInfos[number];
	for (int32 i = 0; i < info.parameter_count; i++) {
		int32 size = info.parameters[i].size; if (size > 8) size = 8;
		if (size <= 0 || info.parameters[i].offset < 0 || info.parameters[i].offset + size > (int32)sizeof(buffer)) continue;
		if (i < 6) memcpy(buffer + info.parameters[i].offset, &regs[i], size);
		else user_memcpy(buffer + info.parameters[i].offset, (void*)(f->r1 + 48 + (i - 6) * 8), size);
	}
	uint64 ret = 0; enable_interrupts(); syscall_dispatcher(number, buffer, &ret); f->r3 = ret; disable_interrupts();
}
extern "C" void ppc64_exception_entry(uint64 vector, iframe* f)
{
	f->vector = vector; Thread* t = thread_get_current_thread(); iframe_stack* s = t != NULL ? &t->arch_info.iframes : &gBootFrameStack; ppc_push_iframe(s, f);
	switch (vector) {
	...
	if (t != NULL) {
		...
		if (t->post_interrupt_callback) { void (*cb)(void*) = t->post_interrupt_callback; void* d = t->post_interrupt_data; t->post_interrupt_callback = NULL; t->post_interrupt_data = NULL; restore_interrupts(state); cb(d); }
...
status_t arch_int_init_post_vm(kernel_args* args)
{
	...
	size_t n = (size_t)(&__irqvec_end - &__irqvec_start); if (n > args->arch_args.exception_handlers.size) return B_BAD_VALUE;
	memcpy(handlers, &__irqvec_start, n); arch_cpu_sync_icache(handlers, n); set_msr(get_msr() & ~MSR_EXCEPTION_PREFIX); return B_OK;
}
