...
void ppc_push_iframe(struct iframe_stack* stack, struct iframe* frame)
	{ ASSERT(stack->index < IFRAME_TRACE_DEPTH); stack->frames[stack->index++] = frame; }
void ppc_pop_iframe(struct iframe_stack* stack)
	{ ASSERT(stack->index > 0); --stack->index; }
...
status_t arch_thread_init(kernel_args*)
	{ memset(&sInitialState, 0, sizeof(sInitialState)); return B_OK; }
status_t arch_team_init_team_struct(Team*, bool) { return B_OK; }
status_t arch_thread_init_thread_struct(Thread* thread)
	{ memcpy(&thread->arch_info, &sInitialState, sizeof(arch_thread)); return B_OK; }
...
void arch_restore_fork_frame(struct arch_fork_arg*)
	{ panic("ppc64: fork restore not implemented"); }
