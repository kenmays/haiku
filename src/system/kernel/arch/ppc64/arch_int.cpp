...
static void handle_syscall(iframe*f){uint8 buffer[MAX_SYSCALL_PARAMETERS*8];memset(buffer,0,sizeof(buffer));uint64 regs[6]={f->r3,f->r4,f->r5,f->r6,f->r7,f->r8};uint32 number=(uint32)f->r0;if(number>=(uint32)kSyscallCount)return;const extended_syscall_info&info=kExtendedSyscallInfos[number];for(int32 i=0;i<info.parameter_count;i++){int32 size=info.parameters[i].size;if(size>8)size=8;if(info.parameters[i].offset<0||info.parameters[i].offset+size>(int32)sizeof(buffer))continue;if(i<6)memcpy(buffer+info.parameters[i].offset,&regs[i],size);else user_memcpy(buffer+info.parameters[i].offset,(void*)(f->r1+48+(i-6)*8),size);}uint64 ret=0;enable_interrupts();syscall_dispatcher(number,buffer,&ret);f->r3=ret;disable_interrupts();}
