...
{
	return apple_g5_platform_init(args);
}
status_t arch_cpu_init_post_vm(kernel_args* args)
{
	return apple_g5_platform_init_post_vm(args);
}
...
{
	ppc_sync();
	for (; start < end; start += B_PAGE_SIZE) tlbie(start);
	tlbsync(); ppc_sync();
}
void arch_cpu_invalidate_tlb_list(intptr_t, addr_t pages[], int count)
{
	ppc_sync();
	for (int i = 0; i < count; i++) tlbie(pages[i]);
	tlbsync(); ppc_sync();
}
...
status_t arch_cpu_user_memcpy(void* to, const void* from, size_t size, addr_t* faultHandler)
{
	...
	memcpy(to, from, size); *faultHandler = old; return B_OK;
error: *faultHandler = old; return B_BAD_ADDRESS;
}
...
ssize_t arch_cpu_user_strlcpy(char* to, const char* from, size_t size, addr_t* faultHandler)
{
	...
	if (size != 0) { size_t n = length < (ssize_t)size - 1 ? length : size - 1; memcpy(to, from, n); to[n] = 0; }
	*faultHandler = old; return length;
error: *faultHandler = old; return B_BAD_ADDRESS;
}

status_t arch_cpu_user_memset(void* address, char value, size_t count, addr_t* faultHandler)
{
	addr_t old = *faultHandler;
	if (ppc_set_fault_handler(faultHandler, (addr_t)&&error)) goto error;
	memset(address, value, count); *faultHandler = old; return B_OK;
error: *faultHandler = old; return B_BAD_ADDRESS;
}
