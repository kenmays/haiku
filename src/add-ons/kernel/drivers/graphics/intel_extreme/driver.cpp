...
const struct supported_device {
	...
	{0x3185, INTEL_MODEL_KBYM, "GeminiLake GT1"},
	{0x3184, INTEL_MODEL_KBYM, "GeminiLake GT1.5"},
...
detect_intel_pch()
{
	...
	for (int32 i = 0; gPCI->get_nth_pci_info(i, &info) == B_OK; i++) {
		if (info.vendor_id != VENDOR_ID_INTEL
			|| info.class_base != PCI_bridge || info.class_sub != PCI_isa)
			continue;

		unsigned short id = info.device_id & INTEL_PCH_DEVICE_ID_MASK;
		switch(id) {
			case INTEL_PCH_IBX_DEVICE_ID: return INTEL_PCH_IBX;
			case INTEL_PCH_CPT_DEVICE_ID:
			case INTEL_PCH_PPT_DEVICE_ID: return INTEL_PCH_CPT;
			case INTEL_PCH_LPT_DEVICE_ID:
			case INTEL_PCH_LPT_LP_DEVICE_ID:
			case INTEL_PCH_WPT_DEVICE_ID:
			case INTEL_PCH_WPT_LP_DEVICE_ID: return INTEL_PCH_LPT;
			case INTEL_PCH_SPT_DEVICE_ID:
			case INTEL_PCH_SPT_LP_DEVICE_ID:
			case INTEL_PCH_KBP_DEVICE_ID: return INTEL_PCH_SPT;
			...
			case INTEL_PCH_CMP2_DEVICE_ID: return INTEL_PCH_CNP;
			case INTEL_PCH_CMP_V_DEVICE_ID: return INTEL_PCH_SPT;
			case INTEL_PCH_ICP_DEVICE_ID:
			case INTEL_PCH_ICP2_DEVICE_ID: return INTEL_PCH_ICP;
			case INTEL_PCH_MCC_DEVICE_ID: return INTEL_PCH_MCC;
			case INTEL_PCH_TGP_DEVICE_ID:
			case INTEL_PCH_TGP2_DEVICE_ID: return INTEL_PCH_TGP;
			case INTEL_PCH_JSP_DEVICE_ID: return INTEL_PCH_JSP;
			case INTEL_PCH_ADP_DEVICE_ID:
			case INTEL_PCH_ADP2_DEVICE_ID:
			case INTEL_PCH_ADP3_DEVICE_ID:
			case INTEL_PCH_ADP4_DEVICE_ID:
			case INTEL_PCH_ADP5_DEVICE_ID: return INTEL_PCH_ADP;
		}
	}
...
init_driver(void)
{
	...
	for (int32 cookie = 0; found < MAX_CARDS;) {
		...
		intel_device_profile profile = intel_get_device_profile(info->device_id,
			gDeviceInfo[found]->device_type, gDeviceInfo[found]->device_identifier);
		gDeviceInfo[found]->generation = intel_get_generation_info(profile.generation);
		gDeviceInfo[found]->pipe_count = profile.pipe_count;
		gDeviceInfo[found]->port_count = profile.port_count;
		gDeviceInfo[found]->capabilities = profile.capabilities;

		dprintf(DEVICE_NAME ": (%" B_PRId32 ") %s, gen %u, pipes %u, ports %u, caps %#" B_PRIx32 ", revision = 0x%x\n",
			found, kSupportedDevices[type].name, profile.generation,
			profile.pipe_count, profile.port_count, profile.capabilities,
			info->revision);
...
uninit_driver(void)
{
	...
	for (int32 index = 0; index < MAX_CARDS && gDeviceNames[index] != NULL;
		index++) {
		free(gDeviceInfo[index] != NULL ? gDeviceInfo[index]->pci : NULL);
