...
static cpu_type cpu_from_pvr(uint32 pvr)
{
	switch ((uint16)(pvr >> 16)) {
		...
		case 0x003c: case 0x0044: return CPU_970FX;
...
static bool get_reg(intptr_t node, phys_addr_t& base, size_t& size)
{
	...
	for (uint32 i = 0; i < sizeCells; i++)
		regionSize = (regionSize << 32) | B_BENDIAN_TO_HOST_INT32(*p++);
	if (base == 0 || regionSize == 0)
		return false;
	size = (size_t)regionSize;
	return true;
}
...
{
	machine_info info;
	if (!detect(info, args)) return B_BAD_VALUE;
...
{
	intptr_t node = 0;
	bool u4 = find_compatible("dart", "u4-dart", node);
	if (!u4 && !find_compatible("dart", "u3-dart", node)) {
		dprintf("apple_g5: no DART node in Open Firmware\n");
		return B_OK;
	}
	phys_addr_t physical;
	size_t size;
	if (!get_reg(node, physical, size)) return B_BAD_VALUE;
	void* mapped = NULL;
	...
	status_t status = AppleG5DART::Init((addr_t)mapped, size, u4);
	if (status != B_OK)
